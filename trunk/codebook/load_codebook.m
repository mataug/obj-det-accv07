...
%INPUT:
%   ANNO_DATA:  this data is generate by load_anno_data.m, a structure list
%                   which has two fields: {'img','mask'}
%   PARA_SC:    parameters for shape context and edge detection
%OUTPUT:
%   CODEBOOK:
%     codebook=
%         codes: [#codes x #sc bins], normalized sum to 1
%        sc_sum: [#codes x 1], sum of shape context feature before
%                   normalization
%     sc_weight: [#codes x #sc bins], each row is a 0/1 mask, 1
%                means the shape context bin is inside the object
%      location: [#codes x 2], the (x, y) of code position in the image
%        relpos: [#codes x 2], the relative (x, y) of the code position
%                   to the center
%        img_id: [#codes x 1], which image it comes from ANNO_DATA
%        obj_id: [#codes x 1], which object it comes from ANNO_DATA
%         scale: [#codes x 1], indicating which scale model codes comes
%                   from, it is used when you have multiple scale objects
%                   currently just ignored
%      annolist: [#img x #obj], store the annotation data, including
%                   {'img','edge','theta','mask'}
%          para: parameter for generating this codebook, equalling to
%                   para_sc in 'set_parameter.m'
%
% Liming Wang, Jan 2008
%


% computer the Shape Context bins' masks
...
for kk=1:nb_img
    for jj=1:nb_obj
        ...
        if(isempty(codebook1))
            warning(sprintf('No feature extracted from img_id=%, obj_id=%d',img_id,jj));
            continue;
        end
