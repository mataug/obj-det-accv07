...
%   RECOG_RESULT: detail detection result, save it for the purpose of
%   debugging later
%
% related paper:
...
if(verbose>1)
    now_time    = cputime;
    fprintf(1,'Voting total: %f\n', now_time-last_time);
    last_time   = now_time;
end

% % % % % force unique one-to-one vote  % % % % % %
...
% % % % % % % suppress across scale at rough same position % % % % % % % %
...
bbox_list       = bbox_list(valid_hypo_idx,:);
mask_heights_list= mask_heights_list(valid_hypo_idx,:);
scale_list      = scale_list(valid_hypo_idx);

if(verbose>3)
    display_hypo_rect(img, [], hypo_list, score_list, bbox_list);
end

% % % % % % % prune by area overlapping % % % % % % % % % % % % % % % % % %
